...
def getLabel(label, label_type="account", input_type="single"):
    ...
    if input_type == "single":
        endOrContinue = input(
            'Type "exit" to end or the name of another label of interest to continue\n'
        )
...
def combineAllJson():
            ...
            with open(savePath + "accounts/{}".format(files)) as f:
                ...
                for address, nameTag in dictData.items():
                    if address not in combinedAccountJSON:
                        combinedAccountJSON[address] = {"name": nameTag, "labels": []}
            ...
            with open(savePath + "tokens/{}".format(files)) as f:
                ...
                for address, nameTag in dictData.items():
                    if address not in combinedTokenJSON:
                        combinedTokenJSON[address] = {"name": nameTag, "labels": []}
    ...
    combinedAllJSON = {
        ...
        for key in set(
            list(combinedAccountJSON.keys()) + list(combinedTokenJSON.keys())
        )
    }
...
ignore_list = [
    ...
    "uniswap",
    "remittance+",
]  # Bugged remittance+ bad link, accidental + it seems
